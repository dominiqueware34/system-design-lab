...
| Campaign season freeze + ref reveal (API) | campaign APIs | `src/lib/campaign-season-status.ts`, `campaign-db` (read-only effective status), start/submit freeze, `GET …/seasons/:id/prompts` | Artifact 7 **reduced** (#10 / PR #27): pure effective status from `ends_at`/`starts_at`; freezes **start + submit** when not live; `referenceDesign` on GET prompts **only when effectively ended**. No hub UI change. DB `status` expire / Queues+EF **not planned** (#28 closed); owner may handle season-end persistence later. **Not** rate limits (#25) or next-season seed (#26). |
