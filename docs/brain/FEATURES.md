...
| Campaign season freeze + ref reveal | campaign APIs + hub + DB job | `src/lib/campaign-season-status.ts` (pure effective status), `campaign-db` read-only, `GET …/prompts`, hub; **job:** `supabase/migrations/…_campaign_expire_seasons.sql` → `campaign_expire_seasons()` | Artifact 7 **reduced** (#10): app freezes start/submit + reveals refs from **timestamps** (no write on request). DB `live→ended` is an **async job outside the Next app** (`SELECT campaign_expire_seasons()` via pg_cron / external cron). **Not** rate limits (#25) or next-season seed (#26). |
